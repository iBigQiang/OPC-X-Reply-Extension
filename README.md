...
2. **请勿把 API Key 写入代码或文档并上传到 GitHub**——本仓库 `.gitignore` 已加防御性规则排除 `*.env` / `*.key` / `*credentials*` / `*api_key*` 等模式，并永久排除 `docs/开发及迭代方案调研报告/` 子目录。
3. API Key 仅保存在浏览器扩展本地存储 `chrome.storage.local`；生成回复时推文内容会发送到你配置的 LLM 服务。
4. 如果 X 页面 DOM 结构更新，按钮位置和填入逻辑可能需要后续适配。
5. 如果接口返回 HTML / 非 JSON，**先检查 Base URL 是否填成首页 / 控制台地址**，再检查接口类型与模型。
6. `额外提示词` 字段只作为风格参考——插件硬性保留"3 条候选 / JSON 数组 / 不带前缀"这三件事不被覆盖。
